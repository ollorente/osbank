...
// @ts-ignore
import EstimateInterface from "@/interfaces/EstimateInterface.js";
// @ts-ignore
  ...
  methods: {
    ...
    async addEstimate() {
      try {
        ...
        const estimate = await EstimateInterface(this.estimate);

        const { data, status } = await EstimateDataService.create(estimate)
          .then(async (response) => {
